(ns engi.attest.ed25519
  "Ed25519 through WebCrypto — the only place in engi that knows what a curve
  is.

  Named under `engi.attest` rather than beside the existing `engi.crypto`
  (which is kotobase's client-side key handling) because what it implements is
  the verifier `engi.attest` takes as a parameter, and a name that says so is
...
(defn resolve-certificate
  ...
  `verify-fn` for `engi.attest/verify-certificate`.
...
(defn verify-certificate!
  "A promise of `engi.attest/verify-certificate`'s answer: nil, or a reason."
  [qc chain-id quorum]
