(ns engi.net
  ...
  `engi.wire` says what a message is. This says how a replica treats the peer
  ...
  1. **Reconnecting immediately.** A peer that is down stays down for a while,
     ...
     `engi.pacemaker` uses for views and for the same reason: it must grow,
  ...
  decides who to spend bandwidth on, not who to believe. `engi.consensus` and
  `engi.sync` decide that, and they do not consult this namespace.")
...
(defn on-bad-message
  "A message that `engi.wire/decode` refused. Past the threshold the peer is
