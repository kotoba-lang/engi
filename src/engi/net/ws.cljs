(ns engi.net.ws
  ...
  `engi.net` decides policy — when to dial, what to queue, when to give up.
  `engi.wire` decides what a message is. This opens sockets and moves bytes,
...
(defn make-node
  ...
  `opts`:
    ...
    :url-of     (fn [peer] \"wss://...\")
    :on-message (fn [peer msg]) — msg is already decoded by engi.wire
    :on-reject  (fn [peer reason]) — optional; a refused frame
    :params     engi.net params
  ...
  (let [ctor (or ctor (websocket-ctor))
    ...
    (when-not ctor
      (throw (ex-info "engi.net.ws: no WebSocket available in this runtime" {})))
