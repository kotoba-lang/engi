(ns engi.pacemaker
  ...
  `engi.consensus` owns the SAFETY rules — quorum arithmetic, the shape of a
...
(defn qc-view
  "The view a QC was formed in. A QC from `engi.consensus` carries a height;
...
(defn higher-qc
  ...
  (cond
    ...
    (>= (or (:engi.qc/height a) 0) (or (:engi.qc/height b) 0)) a
    :else b))
...
(defn extends-locked?
  ...
  (let [locked (:locked-qc state)]
    (or (nil? locked)
        (boolean (ancestor? (:engi.qc/block-hash locked) block)))))
...
(defn safe-to-vote?
  ...
  [state block ancestor?]
  (let [locked (:locked-qc state)
        justify (:engi.block/justify block)]
    (boolean
     (or
...
(defn new-view
  ...
  [state]
  {:engi.nv/witness (:witness state)
   :engi.nv/view (:view state)
   :engi.nv/high-qc (:high-qc state)})

(defn timeout-certificate
  "A TC from `messages`, or nil below quorum.

  Counts DISTINCT witnesses, for the same reason `engi.consensus/qc` does: a
  ...
  [messages quorum]
  (when (seq messages)
    (let [view (:engi.nv/view (first messages))]
      (when-not (every? #(= view (:engi.nv/view %)) messages)
      ...
      (let [distinct-witnesses (set (map :engi.nv/witness messages))]
        (when (>= (count distinct-witnesses) quorum)
          {:engi.tc/view view
           ...
           :engi.tc/high-qc (reduce higher-qc nil (map :engi.nv/high-qc messages))})))))
...
(defn on-timeout-certificate
  ...
  [state tc now params]
  (let [view (inc (:engi.tc/view tc))]
    (-> state
        (assoc :view (max (:view state) view))
        (on-qc (:engi.tc/high-qc tc))
        (assoc :failures 0)
