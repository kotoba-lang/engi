(ns engi.net.server
  ...
  `engi.net.ws` dials. A validator also has to be dialled: with only outbound
  ...
  An accepted peer gets `engi.net`'s backoff-free session (it is already open),
...
(defn attach!
  ...
  `engi.net.ws/make-node`: `:on-message`, `:on-reject`, `:params`, plus
